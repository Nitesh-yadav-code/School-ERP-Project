...
const MarkAttendanceModal = ({ open, onClose, onSuccess }) => {
    ...
    const handleSubmit = async () => {
        if (!formData.classId || !formData.sectionId || !formData.academicYear || !formData.date) {
        ...
        setLoading(true);
        try {
            const enrollmentData = Object.entries(attendance).map(([studentEnrollmentId, status]) => ({
                studentEnrollmentId,
                status,
            }));
                ...
                <SectionBox>
                                ...
                                <Tooltip title="Mark All Present" arrow>
                                    <Button
                                        ...
                                        sx={{
                                            ...
                                            textTransform: "none",
                                            "&:hover": { borderColor: "#22c55e", bgcolor: "#22c55e10" },
                                        }}
                    ...
                    {enrollments.length > 0 && (
                        <Box sx={{ display: "flex", gap: 1.5, mb: 2, flexWrap: "wrap" }}>
                            <SummaryChip variant="total" label={`Total: ${summary.total}`} size="small" />
                            <SummaryChip variant="present" label={`Present: ${summary.present}`} size="small" />
                            <SummaryChip variant="absent" label={`Absent: ${summary.absent}`} size="small" />
                            <SummaryChip variant="leave" label={`Leave: ${summary.leave}`} size="small" />
                        </Box>
                    )}
                    ...
                    {fetchingStudents ? (
                        ...
                        <Box sx={{ maxHeight: 380, overflowY: "auto", pr: 0.5 }}>
                            {enrollments.map((enrollment, index) => (
                                <StudentCard key={enrollment._id} status={attendance[enrollment._id]}>
                                    ...
                                    <Box sx={{ display: "flex", gap: 1 }}>
                                        <Tooltip title="Present" arrow>
                                            <StatusButton
                                                ...
                                                onClick={() => handleStatusChange(enrollment._id, "Present")}
                                            >
                                                <FiCheck size={18} />
                                            </StatusButton>
                                        </Tooltip>
                                        <Tooltip title="Absent" arrow>
                                            <StatusButton
                                                ...
                                                onClick={() => handleStatusChange(enrollment._id, "Absent")}
                                            >
                                                <FiX size={18} />
                                            </StatusButton>
                                        </Tooltip>
                                        <Tooltip title="Leave" arrow>
                                            <StatusButton
                                                ...
                                                onClick={() => handleStatusChange(enrollment._id, "Leave")}
                                            >
                                                <FiMinus size={18} />
                                            </StatusButton>
                                        </Tooltip>
